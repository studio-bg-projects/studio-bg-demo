<form method="get" action="{{ url('/erp/documents') }}">
  <div class="row g-3 mb-3">
    <div class="col">
      <div class="form-floating">
        ...
        <label class="fw-bold mb-2 text-body-highlight" for="f-filter-documentNumber">Номер на документа</label>
      </div>
    </div>
    <div class="col-3 col-md-2 col-xxl-1">
      <div class="form-floating">
        <select class="form-select" id="f-op-documentNumber" name="op[documentNumber]">
          @foreach(App\Http\Controllers\Erp\DocumentsController::$filterOperators as $value => $label)
  ...
  <div class="row g-3 mb-3">
    <div class="col">
      <div class="form-floating">
        <input type="hidden" name="op[type]" value="eq"/>
        <select class="form-select" id="f-filter-type" name="filter[type]">
          <option value="" {{ (request()->filter['type'] ?? '') === '' ? 'selected' : '' }}>Всички</option>
          @foreach (App\Enums\DocumentType::cases() as $type)
            <option value="{{ $type->value }}" {{ (request()->filter['type'] ?? '') === $type->value ? 'selected' : '' }}>
              {{ \App\Services\MapService::documentTypes($type)->labelBg }} [{{ $type->value }}]
            </option>
          @endforeach
        </select>
        <label class="fw-bold mb-2 text-body-highlight" for="f-filter-type">Тип</label>
      </div>
    </div>
  </div>
